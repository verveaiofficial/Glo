    ...
    <div className="post-modal">
      ...
      <div className="post-modal-body">
        {profile?.avatar_url ? (
          ...
          <div className={`avatar ${profile?.avatar_grad || 'av-me'}`} style={{ marginBottom: 8 }}>{(profile?.display_name || 'A')[0]}</div>
        )}
...
export function Stories() {
  ...
  useEffect(() => {
    const load = async () => {
      const s = sb();
      const { data } = await s.from('stories').select('*,profiles(*)').gt('expires_at', new Date().toISOString()).order('created_at');
      setStories(data || []);
    };
    load();
  ...
  useEffect(() => {
    const h = () => { if (!userId) { router.push('/login'); return; } setCompose(true); };
      ...
      <div className="stories">
        <button
          ...
          onClick={() => {
            ...
            if (myGroupIdx >= 0) setView({ g: myGroupIdx, i: 0 });
            ...
            <button key={g.uid} className="story" onClick={() => setView({ g: gi, i: 0 })}>
              <span className={`story-ring ${p?.verified === 'gold' ? 'gold' : ''}`}>
                {p?.avatar_url ? (
                  ...
                  <span className={`avatar ${p?.avatar_grad || 'av-1'}`}>{(p?.display_name || '?')[0]}</span>
                )}
              </span>
      ...
      {cur && view && curGroup && (
        <div id="storyViewer" className={`show ${cur.gradient || 'g1'}`}>
          <div className="sv-progress">
            {curGroup.items.map((_, i) => (
              <i key={i} className={i < view.i ? 'done' : i === view.i ? 'live' : ''} />
            ))}
          </div>
...
export function AccountRow({ acc, following }: { acc: Profile; following: boolean }) {
  ...
  return (
    <div className="account rise">
      ...
      <button className={`follow-btn ${f ? 'on' : ''}`} onClick={() => { if (!userId) { router.push('/login'); return; } setF(!f); toggleFollow(acc.id); toast(f ? `Unfollowed ${acc.display_name}.` : `Following ${acc.display_name}.`); }}>
        {f ? 'Following' : 'Follow'}
      </button>
    </div>
  );
