...
export function PostCard({post,liked,reposted,bookmarked}:{post:Post;liked:boolean;reposted:boolean;bookmarked:boolean}){
  const toast=useToast();const{userId}=useAuth();const router=useRouter();
  const[lk,setLk]=useState(liked);const[lp,setLp]=useState(post.likes_count);const[rp,setRp]=useState(reposted);const[rc,setRc]=useState(post.reposts_count);const[bm,setBm]=useState(bookmarked);
...
export function Compose(){
  const{profile,userId}=useAuth();const router=useRouter();const toast=useToast();
  const[text,setText]=useState('');const[media,setMedia]=useState<string|null>(null);const fileRef=useRef<HTMLInputElement>(null);
  const left=280-text.length;
  if(!userId){
    return <div className="compose" onClick={()=>router.push('/login')} style={{cursor:'pointer'}}>
...
export function Stories(){
  ...
  const{userId}=useAuth();const router=useRouter();
...
export function AccountRow({acc,following}:{acc:Profile;following:boolean}){
  const[f,setF]=useState(following);const toast=useToast();const{userId}=useAuth();const router=useRouter();
