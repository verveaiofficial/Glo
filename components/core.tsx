...
export function Shell({ children }: { children: ReactNode }) {
  ...
  const item = (s: string, label: string, icon: ReactNode) => (
    <button className={`nav-item ${screen === s ? 'active' : ''}`} onClick={() => nav(s)}>{icon}{label}</button>
  );

  return (
    <>
      <div id="backdrop" className={open || notifOpen ? 'show' : ''} onClick={() => { setOpen(false); setNotifOpen(false); }} />
      ...
      <div className="wrap">
        <header>
          <button className={`icon-btn burger ${open ? 'open' : ''}`} onClick={() => setOpen(!open)} aria-label="Menu"><span /><span /><span /></button>
          <div className={screen === 'home' ? 'wordmark' : 'page-title'}>{screen === 'home' ? <>Glo<i>.</i></> : T[screen]}</div>
          <button className="icon-btn" onClick={() => { if (!userId) { router.push('/login'); return; } setOpen(false); setNotifOpen(!notifOpen); }} aria-label="Notifications">
            <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M18 8a6 6 0 0 0-12 0c0 7-3 8-3 8h18s-3-1-3-8" /><path d="M13.7 21a2 2 0 0 1-3.4 0" /></svg>
          </button>
        </header>
        {children}
      </div>
        ...
        <div className="fab-container">
          <div className={`fab-menu ${fabOpen ? 'open' : ''}`}>
            <button className="fab-option" onClick={() => handleCreate('post')}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><path d="M12 20h9" /><path d="M16.5 3.5a2.1 2.1 0 0 1 3 3L7 19l-4 1 1-4z" /></svg>
              Post
            </button>
            <button className="fab-option" onClick={() => handleCreate('story')}>
              <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><circle cx="12" cy="12" r="10" /><path d="m16 12-4-4-4 4M12 16V8" /></svg>
              Story
            </button>
          </div>
