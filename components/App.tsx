...
import { useNav, useAuth, useToast, VerifiedBadge, FollowListDrawer } from './core';
import { PostCard, Stories, AccountRow, Empty, PostModal } from './widgets';
import { logout, updateProfile, uploadMedia, toggleFollow } from '@/app/actions';
  ...
  return (
    <div className="post-modal">
      <div className="post-modal-head">
        ...
        <button className="post-btn" disabled={busy || !text.trim()} onClick={send}>
          {busy ? 'Sending...' : 'Send'}
        </button>
      </div>
...
function UserScreen() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const { data: fm } = await s.from('follows').select('*').eq('follower_id', viewId).eq('following_id', userId || '').maybeSingle();
      setFollowsMe(!!fm);
      ...
      if (rpIds.length) {
        const { data: rpPosts } = await s.from('posts').select('*,profiles(*)').in('id', rpIds);
  ...
  return (
    <>
      ...
      <div className="profile-head">
        <div className="profile-top">
          ...
          <div className="profile-actions">
            ...
            {userId && userId !== viewId && (
              <button className={`pbtn ${following ? 'on' : ''}`} onClick={handleFollow}>
                {following ? 'Following' : followsMe ? 'Follow back' : 'Follow'}
              </button>
            )}
            {userId && userId !== viewId && following && (
              <button className="pbtn" onClick={() => setMsgOpen(true)}>Message</button>
            )}
          </div>
        </div>
        ...
        <div className="p-meta">
          <span>Joined {joined}</span>
          <button><b>{user.following_count ?? 0}</b> Following</button>
          <button><b>{user.followers_count ?? 0}</b> Followers</button>
        </div>
      </div>
      <div className="ptabs">
      ...
      {list.length === 0 ? (
        <div className="pempty">
          <h3>No {tab} yet</h3>
          <p>When they {tab === 'posts' ? 'post' : 'repost'}, it'll show up here.</p>
        </div>
      ) : (
        <div id="feed">
          {list.map((p) => (
