...
function FollowList({ id, type, close }: { id: string; type: 'following' | 'followers'; close: () => void }) {
  ...
  useEffect(() => {
    (async () => {
      const s = sb();
      if (type === 'following') {
        const { data } = await s.from('follows').select('following_id').eq('follower_id', id);
        const ids = (data || []).map((x) => x.following_id);
        if (ids.length) { const { data: ps } = await s.from('profiles').select('*').in('id', ids); setRows(ps || []); } else setRows([]);
      } else {
        const { data } = await s.from('follows').select('follower_id').eq('following_id', id);
