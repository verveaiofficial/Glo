...
function TabsBar() {
  ...
  return (
    <div className="tabs tri">
      <button className={`tab ${idx === 0 ? 'active' : ''}`} onClick={() => { setFeedTab('foryou'); go('home'); }}>
        For You
      </button>
      <button className={`tab ${idx === 1 ? 'active' : ''}`} onClick={() => { setFeedTab('following'); go('home'); }}>
        Following
      </button>
      <button className={`tab ${idx === 2 ? 'active' : ''}`} onClick={() => { setFeedTab('explore'); go('home'); }}>
        Explore
      </button>
...
function ProfileScreen() {
  ...
  useRefresh(async () => {
    ...
    if (rpIds.length) {
      const q = await s.from('posts').select('*,profiles(*)').in('id', rpIds);
      reposts = q.data || [];
    }

    if (lkIds.length) {
      const q = await s.from('posts').select('*,profiles(*)').in('id', lkIds);
      likes = q.data || [];
    }
    ...
    <>
      ...
      <div className="set-card">
        ...
        <form action={logout} className="set-row" style={{ borderBottom: 'none' }}>
          <button type="submit" style={{ color: '#f4212e', background: 'none', border: 'none', font: 'inherit', cursor: 'pointer' }}>
            Log out
          </button>
        </form>
      </div>
